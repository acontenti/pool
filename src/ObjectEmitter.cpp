...
void pool::ObjectEmitter::initialize() {
	llvm::InitializeNativeTarget();
	llvm::InitializeNativeTargetAsmPrinter();
}

std::string pool::ObjectEmitter::emit(const std::unique_ptr <llvm::Module> &module, const std::string &filename) {
	std::string error;
	...
	for (auto &global: Natives::get()->getConstants()) {
		module->getOrInsertGlobal(global->getName(), global->getValueType());
	}
	for (auto &function: Natives::get()->getFunctions()) {
