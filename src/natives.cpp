...
namespace pool {
	...
	void initializeNativeSymbols() {
		...
		natives["Integer.+"] = NativeFun::create({{"this"},
			...
			auto par = other[0];
			if (par->getType() == Integer::TYPE) {
				return IntegerClass->newInstance(self->context, location, {}, value + par->as<Integer>()->value);
			} else if (par->getType() == Decimal::TYPE) {
		...
		natives["Decimal.+"] = NativeFun::create({{"this"},
			...
			auto par = other[0];
			if (par->getType() == Integer::TYPE) {
				return DecimalClass->newInstance(self->context, location, {}, value + par->as<Integer>()->value);
			} else if (par->getType() == Decimal::TYPE) {
				return DecimalClass->newInstance(self->context, location, {}, value + par->as<Decimal>()->value);
			} else if (par->getType() == String::TYPE) {
				return StringClass->newInstance(self->context, location, {},
			...
			if (other[0]->getType() == Fun::TYPE) {
				...
				for (auto &value : array->values) {
					if (fun->params.size() == 1)
						...
						fun->execute(fun, {value, IntegerClass->newInstance(self->context, location, {}, i)}, location);
					else if (fun->params.size() == 3)
						fun->execute(fun, {value, IntegerClass->newInstance(self->context, location, {}, i),
										   array}, location);
