...
class LLVMParserImpl : public LLVMParser {
	size_t intFunIdx = 0;
	...
	llvm::Value *parseNum(PoolParser::NumContext *ast, llvm::Value *contextObject) {
		...
		auto *classNewFunction = LLVMNatives::get()->findFunction(constructorName);
	...
	llvm::Value *parseString(PoolParser::StringContext *ast, llvm::Value *contextObject) {
		...
		auto *classNewFunction = LLVMNatives::get()->findFunction("$String_$new");
		auto *classNew = builder.CreateCall(classNewFunction, {value, contextObject});
		return classNew;
	}
	...
	llvm::Value *parseNative(tree::TerminalNode *ast) {
		...
		auto *native = LLVMNatives::get()->find(id);
		if (native) {
			return native;
	...
	llvm::Value *parseIdentifier(antlr4::Token *token, llvm::Value *contextObject, bool local = false) {
		const auto &id = token->getText();
		auto *objectGetFunction = LLVMNatives::get()->findFunction(local ? "$Object_getLocal" : "$Object_get");
	...
	llvm::Value *parseAssignment(PoolParser::AssignmentContext *ast, llvm::Value *contextObject) {
		...
		auto *setFunction = LLVMNatives::get()->findFunction("$Object_set");
	...
	llvm::Value *parseBlock(PoolParser::BlockContext *ast, llvm::Value *contextObject) {
		...
		auto *classNewFunction = LLVMNatives::get()->findFunction("$Function_$new");
	...
	llvm::Value *parseFunction(PoolParser::FunContext *ast, llvm::Value *contextObject) {
		...
		auto *mallocFunction = LLVMNatives::get()->findFunction("$malloc");
		...
		for (const auto &param: paramsAst) {
			...
			if (param->type) {
				...
				type = LLVMNatives::get()->findConstant("$ObjectClass");
			}
	...
	llvm::Value *parseArray(PoolParser::ArrayContext *ast, llvm::Value *contextObject) {
		...
		auto *classNewFunction = LLVMNatives::get()->findFunction("$Array_$new");
	...
	llvm::Value *parsePar(PoolParser::ParContext *ast, llvm::Value *contextObject) {
		if (const auto &exp = ast->expression()) {
			...
			return LLVMNatives::get()->findConstant("$Null");
		}
	}
	...
	llvm::Function *parseStatements(const vector<PoolParser::StatementContext *> &statements) {
		...
		for (const auto &statement: statements) {
			if (const auto &exp = statement->expression()) {
				...
				result = LLVMNatives::get()->findConstant("$Null");
			}
		}
...
public:
	explicit LLVMParserImpl(const shared_ptr<PoolInstanceImpl> &poolInstance)
			: LLVMParser(poolInstance), builder(*this->llvm_context) {}
	...
	void parseProgram(PoolParser::ProgramContext *ast) override {
		...
		builder.SetInsertPoint(entry);
		// create the module
		auto *moduleNewFunction = LLVMNatives::get()->findFunction("$Module_$new");
		auto *moduleName = builder.CreateGlobalStringPtr(llvm::StringRef(module->getName()));
		auto *moduleNew = builder.CreateCall(moduleNewFunction, {moduleName, fun, LLVMNatives::get()->findConstant("$ModuleClass")});
		// execute the module
		auto *moduleExecuteFunction = LLVMNatives::get()->findFunction("$Module_execute");
